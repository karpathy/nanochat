...
# -----------------------------------------------------------------------------
def main():
    print0(f"Configuration: model_tag={model_tag}, step={step}, hf_path={hf_path}")

    # distributed / precision setup
    ddp, ddp_rank, ddp_local_rank, ddp_world_size, device = compute_init()
    autocast_ctx = torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)

    # Load model and tokenizer from HuggingFace or from local file system
    if hf_path is not None:
        # Load from HuggingFace model hub or local HF model directory
