...
# Random Japanese text (to test Japanese compression)
japanese_text = r"""
人工知能（じんこうちのう、英: artificial intelligence、AI）とは、「『計算』という概念と『コンピュータ』という道具を用いて『知能』を研究する計算機科学の一分野」を指す語。

大規模言語モデル（LLM）は、自然言語処理において革新的な進歩をもたらした技術である。Transformerアーキテクチャに基づき、膨大なテキストデータから言語パターンを学習する。GPT、BERT、LLaMAなどの代表的なモデルは、文章生成、翻訳、要約、質問応答など幅広いタスクに対応できる。

機械学習の基本的な流れは以下の通りである：
1. データの収集と前処理
2. モデルの選択とアーキテクチャ設計
3. 学習（訓練）フェーズ
4. 評価と検証
...
日本語処理においては、形態素解析やサブワードトークナイゼーションが重要な役割を果たす。特にByte Pair Encoding（BPE）は、未知語への対応力と語彙サイズのバランスに優れている。
""".strip()
