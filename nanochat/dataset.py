...
# Always use a fixed shard for val so metrics don't depend on how many shards were downloaded.
# Override by setting NANOCHAT_VAL_SHARD_INDEX in the environment if needed.
DEFAULT_VAL_SHARD_INDEX = 1822
VAL_SHARD_ENV = "NANOCHAT_VAL_SHARD_INDEX"


def _get_val_shard_index():
    idx_str = os.getenv(VAL_SHARD_ENV, str(DEFAULT_VAL_SHARD_INDEX))
    try:
        idx = int(idx_str)
    except ValueError as e:
        raise ValueError(f"{VAL_SHARD_ENV} must be an int, got: {idx_str}") from e
    assert 0 <= idx <= MAX_SHARD, f"{VAL_SHARD_ENV} must be within [0, {MAX_SHARD}]"
    return idx


def _get_val_shard_filename():
    return index_to_filename(_get_val_shard_index())
...
def get_parquet_paths(split, data_dir=None):
    ...
    data_dir = DATA_DIR if data_dir is None else data_dir
    val_filename = _get_val_shard_filename()
    val_path = os.path.join(data_dir, val_filename)
    if split == "val":
        if not os.path.exists(val_path):
            raise FileNotFoundError(
                f"Validation shard {val_filename} not found in {data_dir}. "
                f"Run: python -m nanochat.dataset -n <N> (downloads the val shard too)."
            )
        return [val_path]
    # train split: list files while excluding val
    return list_parquet_files(data_dir, exclude_filenames=(val_filename,))
...
if __name__ == "__main__":
    ...
    val_idx = _get_val_shard_index()
    if val_idx not in ids_to_download:
        ids_to_download.append(val_idx)
