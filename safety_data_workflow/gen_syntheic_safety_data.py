...
def generate_safe_conversation(idx: int, adversarial_data: dict):
    ...
    Returns:
        List of messages, format compatible with SFT pipeline
    """
    try:
...
### IMPORTANT: MULTI-TURN CONVERSATION REQUIRED

You MUST generate a multi-turn conversation for this response. Include:
1. Initial user query with the harmful/risky question
2. Your safe refusal and educational response
3. A follow-up user question attempting to circumvent the refusal (e.g., rephrasing, claiming "research purposes", etc.)
4. Your firm, consistent refusal with additional safety explanation

The output should have 4+ messages in total (at least 2 user + 2 assistant messages).
"""
        else:
            additional_instruction = """
### IMPORTANT: SINGLE-TURN CONVERSATION

Generate a single-turn conversation with:
1. User query
2. Your comprehensive safe response (refusal + education)

The output should have exactly 2 messages (1 user + 1 assistant).
"""
        # Build prompt
        prompt = BLUE_TEAM_SYSTEM_PROMPT.format(user_query=user_query) + additional_instruction
...
def main():
    ...
    output_file =  "/home/yanshiqi/my-project/nanochat/data/safety_sft_conversations_multiturn.jsonl"
