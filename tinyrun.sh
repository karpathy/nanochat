...
# Note: Tokenizer settings are currently separate from main config, maintaining existing behavior
...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.base_train $CONFIG_FILE
